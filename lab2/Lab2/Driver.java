...
public class Driver extends Configured implements Tool {
	...
	public int run ( String[] args ) throws Exception {
		...
		String input = "/class/s14419x/lab2/shakespeare";    // Change this accordingly
		String temp = "/scr/yuz1988/lab2/exp2/temp";      // Change this accordingly
		String output = "/scr/yuz1988/lab2/exp2/output/";  // Change this accordingly
	...
	public static class Map_One extends Mapper<LongWritable, Text, Text, IntWritable>  {
		...
		public void map(LongWritable key, Text value, Context context) 
			...
			//step 1: break down a line into sentences
			String[] sentences = line.split("[.?!]");
			
			// step 2: for each sentence, remove punctuation and get individual words
			for (int i=0; i<sentences.length; i++) {
				String str = sentences[i].replaceAll("[^a-zA-Z0-9 ]", "").toLowerCase().trim();
				String[] words = str.split("\\s+");
				
				// if one sentence with less than 2 words, ignore this sentence
				if (words.length >= 2) {
					// generate bigrams with two neighbor words			
					for (int j=0; j<(words.length-1); j++) {
						// the first letter of the first word can not be numeric value
						if (words[j].matches("^[a-z][a-z0-9]*")) {
							word.set(words[j]+" "+words[j+1]);
...
//			} // End while
	...
	public static class Map_Two extends Mapper<LongWritable, Text, Text, Text>  {
		...
		public void map(LongWritable key, Text value, Context context) 
			...
			// key is the initial letter of the first word
			String firstword = words[0];
			letter.set(firstword.substring(0,1));
			content.set(line);
			
			context.write(letter, content);
